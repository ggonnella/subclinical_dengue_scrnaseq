...
```{r fix_barcodes}
vgm[["GEX"]]$barcode = names(vgm[["GEX"]]$barcode)
```
...
Now filter also the cells without VJ and save them as a separate object

```{r filter_unavailable_vj}
vgm_HL <- vgm_filter_unavailable_vj(vgm)
```
...
print(combined_plot)
ggsave(paste0(plots_dir, "after_filter_unavail_VJ.n_cells_per_sample.pdf"), combined_plot)
```
