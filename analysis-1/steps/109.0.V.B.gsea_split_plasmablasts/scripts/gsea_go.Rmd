---
title: "Inden | B cells | Gene Sets Expression Analysis | GO"
...
plots_dir_sg <- paste0(this_step_dir, "results/celltypes_s5/go/")
dir.create(plots_dir_sg, recursive=T)
for (subdir in c("tables", "barplots", "dotplots", "cnet_plots", "emap_plots",
                "ridgeplots", "gsea_plots")) {
  dir.create(paste0(plots_dir_sg, subdir), recursive=T)
}
...
```{r define_gsea_go_function}
input_tables_dir <- input_tables_dir_s
plots_dir <- plots_dir_sg
...
for (input_file in input_files) {
  ...
  if (nrow(gse) == 0) {
    ...
    fname <- paste0(plots_dir, "tables/", group, ".gsea.GO.tsv")
