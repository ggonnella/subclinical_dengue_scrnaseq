...
so@misc$qc_filtering$nCells_failing_singleR =
  count((!so$pruned.labels_singleR_main %in%
        c("CD4+ T cells", "CD8+ T cells", "T cells")) |
        (!so$pruned.labels_singleR_fine %in%
               c("Naive CD4 T cells", "Naive CD8 T cells",
...
so <- subset(so, subset = (pruned.labels_singleR_main %in%
               c("CD4+ T cells", "CD8+ T cells", "T cells")))
so <- subset(so, subset = pruned.labels_singleR_fine %in%
               c("Naive CD4 T cells", "Naive CD8 T cells",
               "Effector memory CD8 T cells", "Central memory CD8 T cells",
               "T regulatory cells", "Th1 cells", "Vd2 gd T cells",
               "Terminal effector CD8 T cells", "Non-Vd2 gd T cells", 
