...
input_step_dir <- paste0(analysis_root, "steps/108.0.P.B.split_plasmablasts/")
input_var_dir <- paste0(input_step_dir, "results/vars/")
if (!dir.exists(input_var_dir)) {
  stop("Input variable directory does not exist: ", input_var_dir)
}

this_step_dir <- paste0(analysis_root, "steps/108.0.V.B.splitted_plasmablasts/")
...
so@misc$proptest_mod@meta_data$cell_group <- NA
for (group in names(so@misc$fctype_group)) {
  so@misc$proptest_mod@meta_data$cell_group <-
    ifelse(so@misc$proptest_mod@meta_data$pruned.labels_singleR_fine %in%
           so@misc$fctype_group[[group]],
           group, so@misc$proptest_mod@meta_data$cell_group)
}
