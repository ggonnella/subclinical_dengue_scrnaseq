---
title: "InDen T cells | Clustering resolutions comparison"
...
```{r load, echo=F}
so <- readRDS(paste0(var_dir, "integrated_so.rds"))
```
...
for (g.b in resolutions) {
  res = gsub("harmsnn_res.", "", g.b)
  cat("### Resolution : ", res, "\n")
  print(knitr::kable(as.data.frame.matrix(table(so@meta.data[[g.b]],
                                              so$pruned.labels_singleR_fine))))
  print(knitr::kable(as.data.frame.matrix(addmargins(round(prop.table(
          table(so@meta.data[[g.b]], so$pruned.labels_singleR_fine),
            margin = 1)*100, digits = 2), margin = 2))))
  print(knitr::kable(as.data.frame.matrix(addmargins(round(prop.table(
          table(so$pruned.labels_singleR_fine, so@meta.data[[g.b]]),
            margin = 1)*100, digits = 2), margin = 2))))
}
```
...
```{r singleR_and_phase_vs_clust_plots, fig.width=12, fig.height=12, echo=F}
for (g.b in resolutions) {
  res = gsub("harmsnn_res.", "", g.b)
  cat("Resolution : ", res, "\n")
  df <- as.data.frame(round(prop.table(table(so@meta.data[[g.b]],
...
print(ggplot(df, aes(x = phase_cell_type, y = percentage))+
  ...
  df2 <- as.data.frame(round(prop.table(table(interaction(so$Phase,
            so$pruned.labels_singleR_fine), so@meta.data[[g.b]]),
                                        margin = 2)*100, digits = 2))
