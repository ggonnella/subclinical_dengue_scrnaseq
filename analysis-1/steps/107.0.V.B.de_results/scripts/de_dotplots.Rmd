---
title: "InDen B cells | Gene differential expression dot plots"
...
input_step_dir <- paste0(analysis_root, "steps/107.0.P.B.de_analysis/")
input_var_dir <- paste0(input_step_dir, "results/vars/")
if (!dir.exists(input_var_dir)) {
  stop("Input variable directory does not exist: ", input_var_dir)
}

this_step_dir <- paste0(analysis_root, "steps/107.0.V.B.de_results/")
...
for (celltype in unique(so$pruned.labels_singleR_fine.mod)) {
  ...
  print(plot)
  fname <- paste0(plots_outdir, "B.de_genes_dotplot.",
                  celltype_sanitized, ".svg")
  ggsave(fname, plot = plot, device = "svg")
}
```

```{r dot_plots_de_genes_unsplitted, fig.height=8, fig.width=8}
...
plot <- GEX_dotplot(ct_sub, "clinical_condition", de[[celltype]],
                    max_expr = max_expr, title = celltype)
print(plot)
fname <- paste0(plots_outdir, "B.de_genes_dotplot.", celltype_sanitized, ".svg")
ggsave(fname, plot = plot, device = "svg")
```
...
for (celltype in unique(so$pruned.labels_singleR_fine.mod)) {
  ...
  plot <- GEX_dotplot(ct_sub, "clinical_condition", all_de,
                       max_expr = max_expr, title = celltype)
  print(plot)
  fname <- paste0(plots_outdir, "B.all_de_genes_dotplot.", celltype_sanitized, ".svg")
  ggsave(fname, plot = plot, device = "svg")
}
```
