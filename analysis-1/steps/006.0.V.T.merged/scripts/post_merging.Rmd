---
title: "InDen Project | T cells | Data visualization after merging"
...
DimPlot(object = so, reduction = "umap", group.by = "pruned.labels_singleR_main",
        cols = mctype_col, label = TRUE, repel = TRUE, split.by = "orig.ident", ncol = 3) +
  NoLegend() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "orange1") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "orange1")
```
...
celltype_df <- as.data.frame(
              round(prop.table(table(so$orig.ident,
                                     so$pruned.labels_singleR_main),
                               margin = 2)*100, digits = 2))
colnames(celltype_df) <- c("sample_id", "cell_type", "percentage")
```

```{r celltypes_per_sample_1_main, fig.width=12, fig.height=4, echo=F}
ggplot(celltype_df, aes(x = cell_type, y = percentage))+
  geom_bar(stat = "identity", aes(fill = sample_id))+
  scale_fill_manual(values = sample_cols)+
  theme_bw()+
  coord_flip()+
  scale_x_discrete(limits = rev(levels(celltype_df$cell_type)))
```

```{r celltypes_per_sample_2_main, fig.width=12, fig.height=6, echo=F}
ggplot(celltype_df, aes(x = sample_id, y = percentage))+
  geom_bar(stat = "identity", aes(fill = sample_id))+
  scale_fill_manual(values = sample_cols)+
  theme_bw()+
...
DimPlot(object = so, reduction = "umap", group.by = "pruned.labels_singleR_fine",
        cols = fctype_col, label = TRUE, repel = TRUE, split.by = "orig.ident", ncol = 3) +
        NoLegend() +
        geom_hline(yintercept = 0, linetype = "dashed", color = "orange1") +
        geom_vline(xintercept = 0, linetype = "dashed", color = "orange1")
```
...
```{r celltypes_per_sample_1_fine, fig.width=12, fig.height=6, echo=F}
ggplot(celltype_df, aes(x = cell_type, y = percentage))+
  geom_bar(stat = "identity", aes(fill = sample_id))+
  scale_fill_manual(values = sample_cols)+
  theme_bw()+
...
```{r celltypes_per_sample_2_fine, fig.width=12, fig.height=10, echo=F}
ggplot(celltype_df, aes(x = sample_id, y = percentage))+
  geom_bar(stat = "identity", aes(fill = sample_id))+
  scale_fill_manual(values = sample_cols)+
  theme_bw()+
...
print(knitr::kable(as.data.frame.matrix(addmargins(
                      round(prop.table(table(so@meta.data[[g.b]],
                                             so$pruned.labels_singleR_fine),
                          margin = 1)*100, digits = 2), margin = 2))))
print(knitr::kable(as.data.frame.matrix(addmargins(
                      round(prop.table(table(so$pruned.labels_singleR_fine,
                                             so@meta.data[[g.b]]),
                          margin = 1)*100, digits = 2), margin = 2))))
```
...
df2 <- as.data.frame(round(prop.table(table(so$pruned.labels_singleR_fine,
                                            so@meta.data[[g.b]]),
                                      margin = 2)*100, digits = 2))
...
Plots comparing cell type assignments divided by cell phase assignment with the
unsupervised clustering

```{r singleR_and_phase_vs_clustering_plots, fig.width=12, fig.height=12, echo=F}
df <- as.data.frame(round(
          prop.table(table(so@meta.data[[g.b]],
                           interaction(so$Phase, so$pruned.labels_singleR_fine)),
                     margin = 2)*100, digits = 2))
...
print(ggplot(df, aes(x = phase_cell_type, y = percentage))+
  ...
  df2 <- as.data.frame(round(
            prop.table(table(interaction(so$Phase, so$pruned.labels_singleR_fine),
